...
INTERVAL_RE = re.compile(r"\b(?:95%\s*(?:CI|confidence interval)|CI|IQR|prediction interval|credible interval|\d+[\u2013\u2014-]\d+)\b", re.IGNORECASE)
ESTIMATE_RE = re.compile(
    r"\b(?:RR|OR|HR|MD|SMD|RD|AUC|sensitivity|specificity|mean difference|risk ratio|fragility index|median|prevalence|calibration slope)\b",
    re.IGNORECASE,
)
...
def validate(text: str, strict_words: bool = True) -> dict:
    ...
    add(
        ...
        len(sentences) >= 7
        and any(keyword in sentences[6].lower() for keyword in ["limit", "limited", "caution", "boundary", "harm", "scope", "cannot", "may not", "remain", "unclear", "uncertain", "exclude", "extend", "warrant"]),
